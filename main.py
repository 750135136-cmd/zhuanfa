...
media_group_wait_time = 1.5  # 媒体组等待时间（秒），确保所有媒体都接收完成
processed_msg_ids = set()  # 已转发消息ID缓存
max_cache_size = 1000  # 缓存最大容量，避免内存溢出
media_group_cache = {}  # 媒体组临时缓存，用于合并同组的多图/多视频
...
async def process_media_group(grouped_id):
    ...
    # 2. 核心规则1：纯文字消息直接拦截（媒体组必然带媒体，此条仅兜底）
    if not any(msg.media for msg in msg_list):
        print(f"⏭️  已拦截 | 源：{source_name} | 原因：媒体组无有效媒体")
        return
    ...
    async with TelegramClient(session_name, api_id, api_hash) as client:
        ...
        async def handler(event):
            ...
            if grouped_id:
                async with media_group_lock:
                    ...
                    if grouped_id not in media_group_cache:
                        ...
                        # 启动超时处理，等待所有媒体接收完成
                        asyncio.create_task(
                            asyncio.sleep(media_group_wait_time)
                            .then(lambda: process_media_group(grouped_id))
                        )
                    # 将当前媒体加入对应媒体组缓存
            ...
            try:
                ...
                print(f"❌ 单媒体转发失败 | 源：{source_name} | 目标：{target_channel} | 错误详情：{e}")
