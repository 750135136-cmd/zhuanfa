...
# 监听-目标频道配对，优先用频道ID，稳定性最高
channels = [
    {
        'source': '@wenan77',  # 可直接替换为频道ID
        'target': '@wnffx',
    },
    {
        'source': '@sucai75',
...
# ========== 每日转发限额配置 ==========
DAILY_MAX_FORWARD = 10  # 每个源频道每日最多转发条数，可自定义修改
# ==========================================
processed_msg_ids = set()
max_cache_size = 1000
media_group_cache = {}
media_group_lock = asyncio.Lock()
count_lock = asyncio.Lock()  # 计数并发锁，避免计数错误

# ========== 内存计数管理（无文件） ==========
# 初始化内存计数变量
current_count_date = datetime.now().strftime("%Y-%m-%d")
daily_forward_count = {}  # 格式：{源频道ID: 当日已转发条数}

async def get_today_count(source_id):
    """获取指定源频道当日已转发条数，自动跨天重置"""
...
async def main():
    async with TelegramClient(session_name, api_id, api_hash) as client:
        ...
        print(f"✅ 每日转发限额功能已启动，每个源频道每日最多转发 {DAILY_MAX_FORWARD} 条（内存计数，重启后清零）")
